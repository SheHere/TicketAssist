...
<head>
	...
	<script>
        $( document ).ready(function() {
            $( "#notificationBubble" ).parent().html();
        };
	</script>
</head>
...
if($num_notifications > 0){
	$bubble_script = '   
        <script>
			$( document ).ready(function() {
				$( "#notificationBubble" ).parent().html("<span class="badge">' . $num_notifications . '</span>);
			});";
		</script>';
}

//Build notification page with non-dismissed or all notifications, depending on $_GET
...
if (mysqli_num_rows($result) > 0) {
	...
	$printConcat = "";
	if (isset($_GET['get'])) {
		$printConcat .= '<br><p style="text-align: center"><a target="_self" href="https://140.209.47.120/notifications/Notifications.php">Show less Notifications</a></p><hr>';
	}
