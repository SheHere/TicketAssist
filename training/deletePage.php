...
$query = "DELETE FROM training_pages WHERE title = '$toDelete';";
$result = mysqli_query($con,$query);
if(!$result) {
	//Insert something that would happen if the information was not placed in
	//the database correctly.
	echo '<script>parent.errorAlert("'.mysqli_error($con).'","https://tdta.stthomas.edu/training/Page.php?title='.str_replace(' ', '', $toDelete).'")</script>';
} else {
